...
Open WebUI 的原生 function calling 会默认给基础模型附加知识库、聊天、任务等内置工具。当前 CPU 后端处理这份约 6K-token 的隐藏工具清单需要接近 2 分钟，因此初始化时通过全局模型元数据关闭 `builtin_tools`，并把课程助教设为默认模型。课程模型继续使用 legacy function calling 与自己的题库工具，RAG 和五项题库功能不依赖这些通用内置工具。
